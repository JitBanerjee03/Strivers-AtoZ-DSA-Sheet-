...
public:
    ...
    int Recursion(int index,vector<int> &nums,int sum1,int k1,int sum2,int k2,vector<vector<vector<int>>> &dp){
       ...
       if(dp[index][abs(sum1)][abs(sum2)]!=-1)  //Base case 3 in case of memoiation
       return dp[index][abs(sum1)][abs(sum2)];

       int s1=k1,s2=k2;
       int first=Recursion(index-1,nums,sum1+nums[index],++s1,sum2,k2,dp);
       int second=Recursion(index-1,nums,sum1,k1,sum2+nums[index],++s2,dp);

       return dp[index][abs(sum1)][abs(sum2)]=min(first,second);
    }
    ...
    int minimumDifference(vector<int>& nums) {
      ...
      return Recursion(nums.size()-1,nums,0,0,0,0,dp);
    }
};
