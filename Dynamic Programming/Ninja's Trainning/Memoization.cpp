...
*/
int Recursion(int rows,int previousActivity,vector<vector<int>> &points,vector<vector<int>> &dp){  //Here 
   ...
   if(dp[rows][previousActivity]!=-1)
   return dp[rows][previousActivity];
   
   int first=0,second=0,third=0;
   
   if(previousActivity!=2)
   first=points[rows][2]+Recursion(rows-1,2,points,dp);

   if(previousActivity!=1)
   second=points[rows][1]+Recursion(rows-1,1,points,dp);

   if(previousActivity!=0)
   third=points[rows][0]+Recursion(rows-1,0,points,dp);

   return dp[rows][previousActivity]=max(max(first,second),third);
}
