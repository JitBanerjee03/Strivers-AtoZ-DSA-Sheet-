...
class Solution {
public:
    //Using memoization a dynamic programming technique to resolve the overlapping recursive function call to reduce the time complexity
    int Recursion(int i,int j,vector<vector<int>> &matrix,vector<vector<int>> &dp,int n){
      ...
      if(j-1>=0)
      leftDiagonal=Recursion(i-1,j-1,matrix,dp,n);

      vertical=Recursion(i-1,j,matrix,dp,n);

      if(j+1<n)
      rightDiagonal=Recursion(i-1,j+1,matrix,dp,n);

      return dp[i][j]=matrix[i][j]+min(min(leftDiagonal,rightDiagonal),vertical);
    }

    int minFallingPathSum(vector<vector<int>>& matrix) {
      int n=matrix.size();  //n is the size of the matrix
      vector<vector<int>> dp(n,vector<int>(n,-1));  //declaring the dp 2D vector for memoization
      int ans=Recursion(n-1,0,matrix,dp,n);
      for(int i=1;i<n;i++)
        ans=min(ans,Recursion(n-1,i,matrix,dp,n));
